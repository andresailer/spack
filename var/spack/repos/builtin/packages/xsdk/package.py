...
class Xsdk(BundlePackage):
    ...
    depends_on('trilinos@13.0.0+hypre~superlu-dist+metis+hdf5~mumps+boost~suite-sparse+tpetra+nox+ifpack2+zoltan2+amesos2~exodus~dtk+intrepid2+shards', #superlu,dtk
               when='@0.6.0 +trilinos')
    ...
    depends_on('strumpack@4.1.0-xsdk~cuda', when='@0.6.0 +strumpack')
    ...
    #depends_on('phist@1.9.0 ~fortran ~scamac ~openmp ~host ~int64', when='@0.6.0 +phist')
